...
def load_data_from_lmdb(batch_size, dataset='Faces_emore'):
    ...
    if dataset == 'CASIA':
        ...
        print('ooooooooooooooooooo1111111oooooooo')
    ...
    dataloaders = {'train_dataset': data.DataLoader(dataset_train, batch_size=batch_size, num_workers=16,pin_memory=True, shuffle=True),
                   
                   'LFW': data.DataLoader(dataset_LFW, batch_size=batch_size,pin_memory=True, shuffle=False, num_workers=4),
                   'CFP_FP': data.DataLoader(dataset_CFP_FP, batch_size=batch_size,pin_memory=True, shuffle=False, num_workers=4),
                   'AgeDB30': data.DataLoader(dataset_AgeDB30, batch_size=batch_size,pin_memory=True, shuffle=False, num_workers=4)}
...
def load_muti_data_train_fix_from_lmdb(batch_size, dataset = 'Faces_emore'):
    ...
    if dataset == 'CASIA':
        ...
        path = "/data/face_dataset/ms1m_lmdb/ms1m.lmdb"
        dataset_train_ms1m = ImageFolderLMDB(path, train_transform) 
        
    else:
        raise NameError('no training data exist!')

    # 顺德数据集
    path = "/data/face_dataset/privacy_data.lmdb"
    dataset_train_shunde = ImageFolderLMDB(path, train_transform) 

    # glintface
    path = "/data/face_dataset/source_glint.lmdb"
    dataset_train_glint = ImageFolderLMDB(path, train_transform)

    # dataset_size = len(dataset_train_ms1m)
    
    dataloaders = {'ms1m_dataset': DataLoaderX(dataset_train_ms1m, batch_size=batch_size,pin_memory=True, shuffle=True,num_workers=16),
                    'shunde_dataset': DataLoaderX(dataset_train_shunde, batch_size=150,pin_memory=True, shuffle=True,num_workers=16),
                    'glint_dataset': DataLoaderX(dataset_train_glint, batch_size=150,pin_memory=True, shuffle=True,num_workers=16),
                    'privacy': DataLoaderX(privacy, batch_size=batch_size,pin_memory=True, shuffle=False, num_workers=4),
                    'LFW': DataLoaderX(dataset_LFW, batch_size=batch_size,pin_memory=True, shuffle=False,num_workers=4),
                    'CFP_FP': DataLoaderX(dataset_CFP_FP, batch_size=batch_size,pin_memory=True, shuffle=False,num_workers=4),
                    'AgeDB30': DataLoaderX(dataset_AgeDB30, batch_size=batch_size,pin_memory=True, shuffle=False,num_workers=4)}

    dataset = {'LFW': dataset_LFW,'privacy':privacy,
               'CFP_FP': dataset_CFP_FP, 'AgeDB30': dataset_AgeDB30}
    
    dataset_sizes = {'LFW': len(dataset_LFW),'privacy':len(privacy),
                     'CFP_FP': len(dataset_CFP_FP), 'AgeDB30': len(dataset_AgeDB30)}
...
def load_data_with_MXNet_dali(batch_size ,args ,dataset = 'Faces_emore'):
    ...
    if dataset == 'CASIA':
        ...
        pipes = MXNetReaderPipeline(path,batch_size=batch_size, num_threads=8, device_id = args.local_rank, num_gpus = torch.cuda.device_count()) 
        pipes.build()
