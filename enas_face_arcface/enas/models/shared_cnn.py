...
class ENASLayer(nn.Module):
    ...
    def __init__(self, layer_id, in_planes, out_planes):
        ...
        self.branch_0 = ConvBranch(in_planes, out_planes, kernel_size=3, separable=True)
        self.branch_1 = ConvBranch(in_planes, out_planes, kernel_size=5, separable=True)
        self.branch_2 = PoolBranch(in_planes, out_planes, 'avg')
        self.branch_3 = PoolBranch(in_planes, out_planes, 'max')
...
class SharedCNN(nn.Module):
    def __init__(self,
        ...
        self.bn_o1 = nn.BatchNorm2d(self.out_filters)
        self.dropout = nn.Dropout(p=1. - self.keep_prob)
        # self.embbeding = nn.Linear(self.out_filters, 128)
        self.embbeding = nn.Linear(self.out_filters*4*4, 512)
        self.bn_o2 = nn.BatchNorm1d(512)
        self.out = nn.Linear(512,85742)
    ...
    def forward(self, x, sample_arc):
        ...
        x = self.embbeding(x)
        embbeding = self.bn_o2(x)
        out = self.out(embbeding)
        
        
        return out,embbeding

class FixSharedCNN(nn.Module):
    def __init__(self,
                 num_layers=12,
                 num_branches=4,
                 out_filters=24,
        ...
        super(FixSharedCNN, self).__init__()
    ...
    def forward(self, x, sample_arc,label=None):
