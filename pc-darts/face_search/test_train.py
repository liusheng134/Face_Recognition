...
def train(train_queue, valid_queue, model, margin, optimizer, optimizer_a, criterion, lr,epoch):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if epoch >=args.begin:
            ...
            thetas,loss_a = margin(embbeding,target_search)

            # loss_a = margin.coupute_loss(thetas, target_search)
            loss_a.sum().backward()
            if MULTI_GPU:
        ...
        thetas,loss = margin(embbeding,target)

        # loss = margin.coupute_loss(thetas, target)
...
def train_with_dali(train_queue, model, margin, optimizer, optimizer_a, criterion, lr,epoch):
    ...
    for i, data  in enumerate(train_queue):
        ...
        if epoch >=args.begin and step % 2 ==0:
            optimizer_a.zero_grad()
            embbeding = model(input)
            thetas,loss_a = margin(embbeding,target)

            # loss_a = margin.coupute_loss(thetas, target)
...
if __name__ == '__main__':
    ...
    mp.spawn(main_worker, nprocs=3, args=(1, args))
